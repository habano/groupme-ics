...
def groupme_json_to_ics(groupme_json):
    ...
    for event in groupme_json.get('response', {}).get('events', []):
        ...
        start_time = datetime.fromisoformat(event.get('start_at').replace('Z', '+00:00')).astimezone(tz)
        ical_event.add('dtstart', start_time)
        end_time = datetime.fromisoformat(event.get('end_at', start_time.isoformat()).replace('Z', '+00:00')).astimezone(tz)
        ...
        updated_at = datetime.fromisoformat(event.get('updated_at', start_time.isoformat()).replace('Z', '+00:00')).astimezone(tz)
        ical_event.add('last-modified', updated_at)
        ical_event.add('dtstamp', updated_at)
